...
QUOTES =
{
  AnselAdams = require 'ELIA.data.AnselAdams',
  BenjaminFranklin = require 'ELIA.data.BenjaminFranklin',
  MarkTwain = require 'ELIA.data.MarkTwain',
  Roman = require 'ELIA.data.Roman',
  TeddyRoosevelt = require 'ELIA.data.TeddyRoosevelt',
  ThomasJefferson = require 'ELIA.data.ThomasJefferson',
  WillRogers = require 'ELIA.data.WillRogers',
  WinstonChurchill = require 'ELIA.data.WinstonChurchill',
}
...
{ 
  {
    ...
    create = function()
      ...
      -- ELIA.states[1].vars.startOrigin = bullet.btVector3(njlic.SCREEN():x() + horiz_margin, njlic.SCREEN():y() - (ELIAFont:maxLineHeight() + vert_margin), -1)
      ELIA.states[1].vars.startOrigin = bullet.btVector3(njlic.SCREEN():x() + horiz_margin, njlic.SCREEN():y() - ELIAFont:maxLineHeight() - vert_margin, -1)
    ...
    destroy = function()
      ...
      njlic.Node.destroy(ELIA.states[1].vars.accuracyNode)
      ...
      njlic.PhysicsShapeBox.destroy(physicsShape)
      physicsShape = nil

      njlic.PhysicsBodyRigid.destroy(physicsBody)
      physicsBody = nil

      njlic.Node.destroy(ELIA.states[1].vars.doneNode)
  ...
  },
  {
    ...
    create = function()
      -- print("created splash")

      ELIA.states[2].vars.titleNode = DrawTitle(nil, "ELIA")
      table.insert(ELIA.states[2].vars.nodes, ELIA.states[2].vars.titleNode)

      ELIA.states[2].vars.endlessLetterNode = DrawEndlessLetterButton(njlic.SCREEN():x() * 0.5, (njlic.SCREEN():y() * 0.5) - 100, ELIA.states[2].vars.endlessLetterNode)
      table.insert(ELIA.states[2].vars.nodes, ELIA.states[2].vars.endlessLetterNode)

      ELIA.states[2].vars.learnMoreNode = DrawLearnMoreButton(njlic.SCREEN():x() * 0.5, (njlic.SCREEN():y() * 0.5) - 195, ELIA.states[2].vars.learnMoreNode)
      table.insert(ELIA.states[2].vars.nodes, ELIA.states[2].vars.learnMoreNode)

      ELIA.states[2].vars.leaderboardNode = DrawLeaderboardButton(njlic.SCREEN():x() * 0.5, (njlic.SCREEN():y() * 0.5) - 290, ELIA.states[2].vars.leaderboardNode)
    ...
    mouseUp = function(rayContact)
      ...
      elseif nodeTag == "learn more node" then
        if ELIA.states[2].vars.learnMoreButtonDown then
          ...
          njlic.World.getInstance():getWorldResourceLoader():openBrowser(learn_more_url)
        end
      elseif nodeTag == "leaderboard node" then
...

  {
    ...
    enter = function()
      ...
      if ELIA.states[3].vars.placeGraphic then
        ...
        if place >= 1 and place <= 10 then
          local vert_margin = njlic.SCREEN():y() / 30.0
          ELIA.states[3].vars.placeGraphic = DrawPlaceGraphic((njlic.SCREEN():x() * 0.5) + (rect.width * 0.5) + 70, vert_margin + vert_margin + 5, ELIA.states[3].vars.placeGraphic, place)
        else
    ...
    create = function()
      ...
      ELIA.states[3].vars.learnMoreNode = DrawLearnMoreButton(njlic.SCREEN():x() * 0.5, (njlic.SCREEN():y() * 0.5) + 95, ELIA.states[3].vars.learnMoreNode)
      table.insert(ELIA.states[3].vars.nodes, ELIA.states[3].vars.learnMoreNode)

      ELIA.states[3].vars.replayNode = DrawReplayButton(njlic.SCREEN():x() * 0.5, (njlic.SCREEN():y() * 0.5) , ELIA.states[3].vars.replayNode)
      table.insert(ELIA.states[3].vars.nodes, ELIA.states[3].vars.replayNode)

      ELIA.states[3].vars.quitNode = DrawQuitButton(njlic.SCREEN():x() * 0.5, (njlic.SCREEN():y() * 0.5) - 95, ELIA.states[3].vars.quitNode)
...

  {
    ...
    create = function()
      ...
      ELIA.states[4].vars.doneNode = DrawDoneButton((njlic.SCREEN():x() * 0.5), 0 + vert_margin + vert_margin, ELIA.states[4].vars.doneNode)
...
function DrawDoneButton(x, y, node)
  ...
  local donePhysicsShape = njlic.PhysicsShapeBox.create()

  local donePhysicsBody = njlic.PhysicsBodyRigid.create()
...
function DrawTitle(node, str)
  ...
  local horiz_margin = njlic.SCREEN():x() / 40.0
  local half_horizontal = njlic.SCREEN():x() * 0.5
  local half_vertical = njlic.SCREEN():y() * 0.5
  
  local node_ret, rect = DrawTitleHelper(arg)
  node_ret:setOrigin(bullet.btVector3(bullet.btVector3(half_horizontal - (rect.width * 0.5), njlic.SCREEN():y() - rect.height - vert_margin, -1)))
  node_ret:show(OrthographicCameraNode:getCamera())
...
local Destroy = function()
  ...
  if PerspectiveCameraNode then
    ...
    njlic.Camera.destroy(camera)
    
    njlic.Node.destroy(PerspectiveCameraNode)
    PerspectiveCameraNode = nil
  end
  ...
  if OrthographicCameraNode then
    ...
    njlic.Node.destroy(OrthographicCameraNode)
    OrthographicCameraNode = nil
  end
