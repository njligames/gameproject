...
function for_each(...)
    ...
    local nodes = arg.nodes or {}
    local action = arg.action or nil
    local param = arg.param or {}

    assert(action, "There must be a action in the table")

    for k,v in pairs(nodes) do
        local status, err = pcall(action, {v, param})
...
function HideNodes(...)
    ...
    local action = function(...)
        ...
        local node = arg[1] or nil
        local param = arg[2] or param

        local camera = param[1] or nil

        assert(node, "There must be node in the table")
        assert(camera, "There must be a camera in the table")
    ...
    for_each( {nodes = nodes, action = action, param = {camera}})
